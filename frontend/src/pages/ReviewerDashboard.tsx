...
import { Building2, CheckCircle, XCircle, Clock, LogOut, FileText } from 'lucide-react';
...
export const ReviewerDashboard: React.FC = () => {
  ...
  // No per-row history modal/expand
  const [clearLoading, setClearLoading] = useState(false);
  const [recentSubmissions, setRecentSubmissions] = useState<Request[]>([]);
  // Clear history for Reviewer role
  const handleClearHistory = async () => {
    if (!window.confirm('Are you sure you want to clear all Reviewer history?')) return;
    setClearLoading(true);
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_BASE_URL}/requests/clear-history/reviewer`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
        fetchRequests();
        alert('Reviewer history cleared!');
      } else {
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setAllRequests(data);
        setRequests(data.filter((r: Request) => r.workflow && r.workflow.reviewer && (r.workflow.reviewer.status === 'Pending' || r.workflow.reviewer.status === 'Under Review')));
        setRecentSubmissions(
          data.filter((r: Request) =>
            r.workflow && r.workflow.reviewer &&
            (r.workflow.reviewer.status === 'Approved' || r.workflow.reviewer.status === 'Rejected') &&
            r.workflow.reviewer.reviewedAt &&
            r.workflow.reviewer.comment !== undefined
          ).sort((a: Request, b: Request) => new Date(b.workflow.reviewer.reviewedAt || '').getTime() - new Date(a.workflow.reviewer.reviewedAt || '').getTime())
        );
      } else {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        fetchRequests();
        // Also refresh after a short delay to ensure backend update is reflected
        setTimeout(() => {
          fetchRequests();
        }, 500);
      } else {
  ...
  const getChecklistStatusBadge = (status: Request['checklistStatus']) => {
  ...
  const getChecklistStatusIcon = (status: Request['checklistStatus']) => {
    switch (status) {
      case 'complete':
  ...
  const getCompletedItemsCount = (checklist: Request['checklist']) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900">{allRequests.filter(r => r.workflow && r.workflow.reviewer && (r.workflow.reviewer.status === 'Pending' || r.workflow.reviewer.status === 'Under Review')).length}</h3>
          ...
          <Card className="text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900">{allRequests.filter(r => r.workflow && r.workflow.reviewer && r.workflow.reviewer.status === 'Approved').length}</h3>
          ...
          <Card className="text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900">{allRequests.filter(r => r.workflow && r.workflow.reviewer && r.workflow.reviewer.status === 'Rejected').length}</h3>
              ...
              <tbody>
                {requests.map((request) => (
                  <tr key={request._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-gray-600">{request.developer?.name || 'N/A'}</td>
                    <td className="py-3 px-4">
                      <div className="flex items-center space-x-2">
                        {getChecklistStatusIcon(request.checklistStatus)}
                        <span className={getChecklistStatusBadge(request.checklistStatus)}>
                          {getCompletedItemsCount(request.checklist)}/6 Complete
                        </span>
                      </div>
                    </td>
              ...
              <tbody>
                {recentSubmissions.length === 0 ? (
                  ...
                  <tr key={r._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${r.workflow.reviewer.status === 'Approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{r.workflow.reviewer.status}</span>
                    </td>
                    <td className="py-3 px-4">{r.workflow.reviewer.comment || '-'}</td>
                    <td className="py-3 px-4">{r.workflow.reviewer.reviewedAt ? new Date(r.workflow.reviewer.reviewedAt).toLocaleString() : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
