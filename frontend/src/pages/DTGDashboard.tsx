...
export const DTGDashboard: React.FC = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setAllRequests(data);
        setRequests(data.filter((r: Request) => r.workflow && r.workflow.dtg && (r.workflow.dtg.status === 'Pending' || r.workflow.dtg.status === 'Under Review')));
        setRecentSubmissions(
          data.filter((r: Request) =>
            r.workflow && r.workflow.dtg &&
            (r.workflow.dtg.status === 'Approved' || r.workflow.dtg.status === 'Rejected') &&
            r.workflow.dtg.reviewedAt &&
            r.workflow.dtg.comment !== undefined
          ).sort((a: Request, b: Request) => new Date(b.workflow.dtg.reviewedAt || '').getTime() - new Date(a.workflow.dtg.reviewedAt || '').getTime())
        );
      } else {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">DTG Dashboard</h1>
                <p className="text-sm text-gray-600">Digital Technology Group Review</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900">{allRequests.filter(r => r.workflow && r.workflow.dtg && (r.workflow.dtg.status === 'Pending' || r.workflow.dtg.status === 'Under Review')).length}</h3>
          ...
          <Card className="text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900">{allRequests.filter(r => r.workflow && r.workflow.dtg && r.workflow.dtg.status === 'Approved').length}</h3>
          ...
          <Card className="text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900">{allRequests.filter(r => r.workflow && r.workflow.dtg && r.workflow.dtg.status === 'Rejected').length}</h3>
                  ...
                  <tr key={request._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                        ...
                        HOD: {request.workflow.hod.status}
                      </span>
                    </td>
              ...
              <tbody>
                {recentSubmissions.length === 0 ? (
                  ...
                  <tr key={r._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${r.workflow.dtg.status === 'Approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{r.workflow.dtg.status}</span>
                    </td>
                    <td className="py-3 px-4">{r.workflow.dtg.comment || '-'}</td>
                    <td className="py-3 px-4">{r.workflow.dtg.reviewedAt ? new Date(r.workflow.dtg.reviewedAt).toLocaleString() : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
