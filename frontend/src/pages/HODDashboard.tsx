...
import { Building2, CheckCircle, XCircle, Clock, AlertTriangle, LogOut } from 'lucide-react';
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setAllRequests(data);
        setRequests(data.filter((r: Request) => r.workflow && r.workflow.hod && (r.workflow.hod.status === 'Pending' || r.workflow.hod.status === 'Under Review')));
        setRecentSubmissions(
          data.filter((r: Request) =>
            r.workflow && r.workflow.hod &&
            (r.workflow.hod.status === 'Approved' || r.workflow.hod.status === 'Rejected') &&
            r.workflow.hod.reviewedAt &&
            r.workflow.hod.comment !== undefined
          ).sort((a: Request, b: Request) => new Date(b.workflow.hod.reviewedAt || '').getTime() - new Date(a.workflow.hod.reviewedAt || '').getTime())
        );
      } else {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900">{allRequests.filter(r => r.workflow && r.workflow.hod && (r.workflow.hod.status === 'Pending' || r.workflow.hod.status === 'Under Review')).length}</h3>
          ...
          <Card className="text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900">{allRequests.filter(r => r.workflow && r.workflow.hod && r.workflow.hod.status === 'Approved').length}</h3>
          ...
          <Card className="text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900">{allRequests.filter(r => r.workflow && r.workflow.hod && r.workflow.hod.status === 'Rejected').length}</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-left py-3 px-4 font-medium text-gray-700">Reviewer Status</th>
                  ...
                  <tr key={request._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                        ...
                        {request.workflow.reviewer.status}
                      </span>
                    </td>
              ...
              <tbody>
                {recentSubmissions.length === 0 ? (
                  ...
                  <tr key={r._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${r.workflow.hod.status === 'Approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{r.workflow.hod.status}</span>
                    </td>
                    <td className="py-3 px-4">{r.workflow.hod.comment || '-'}</td>
                    <td className="py-3 px-4">{r.workflow.hod.reviewedAt ? new Date(r.workflow.hod.reviewedAt).toLocaleString() : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
